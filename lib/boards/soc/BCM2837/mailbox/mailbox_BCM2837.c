...
//
void bcm2837_mailbox_vc_write(uint32_t addres28, uint8_t channel) {
    while (MAILBOX_VC_2837->STATUS & MAIL_FULL); // Wait till the mailbox is empty
    dmb();
    MAILBOX_VC_2837->WRITE = (addres28 | (channel & 0xF)); // Combine data (28:0) with channel (3:0 bits)
}
...
uint32_t bcm2837_get_firmware_version() {
    ...
    bcm2837_mailbox_vc_write(8, (uint32_t)mailbox_buffer | 0x40000000);
    bcm2837_mailbox_vc_read(8);             // Wait for response 
...
uint32_t bcm2837_get_board_model() {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Total size of the buffer in bytes
...
uint32_t bcm2837_get_board_revision() {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Total size of the buffer in bytes
    mailbox_buffer[1] = MAIL_PROC_REQ;      // Request code (0 = request)
...
mac_addr bcm2837_get_mac_address() {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Total size of the buffer in bytes
    mailbox_buffer[1] = MAIL_PROC_REQ;      // Request code (0 = request)
...
board_serial bcm2837_get_board_serial() {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Total size of the buffer in bytes
    mailbox_buffer[1] = MAIL_PROC_REQ;      // Request code (0 = request)
...
void bcm2837_get_arm_memory(uint32_t *base, uint32_t *size) {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Total size of the buffer in bytes
    mailbox_buffer[1] = MAIL_PROC_REQ;      // Request code (0 = request)
...
uint32_t bcm2837_get_clock_rate(clock_id id) {
...
void bcm2837_get_clock_rates(uint32_t *id) {
    for (clock_id nr = EMMC_id; nr < CLOCK_SIZE; nr++) {
        id[nr] = bcm2837_get_clock_rate(nr);
    }
}

uint32_t bcm2837_get_clock_rate_measured(clock_id id) {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Buffer grootte
    mailbox_buffer[1] = MAIL_PROC_REQ;
...
void bcm2837_get_clock_rates_measured(uint32_t *id) {
    for (clock_id nr = EMMC_id; nr < CLOCK_SIZE; nr++) {
        id[nr] = bcm2837_get_clock_rate_measured(nr);
    }
}

uint32_t bcm2837_get_max_clock_rate(clock_id id) {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Buffer grootte
    mailbox_buffer[1] = MAIL_PROC_REQ;
...
void bcm2837_get_max_clock_rates(uint32_t *id) {
    for (clock_id nr = EMMC_id; nr < CLOCK_SIZE; nr++) {
        id[nr] = bcm2837_get_max_clock_rate(nr);
    }
}

uint32_t bcm2837_get_min_clock_rate(clock_id id) {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Buffer grootte
    mailbox_buffer[1] = MAIL_PROC_REQ;
...
void bcm2837_get_min_clock_rates(uint32_t *id) {
    for (clock_id nr = EMMC_id; nr < CLOCK_SIZE; nr++) {
        id[nr] = bcm2837_get_min_clock_rate(nr);
    }
}

uint32_t bcm2837_get_soc_temperature(void) {
    mailbox_buffer[0] = sizeof(mailbox_buffer); // Buffer grootte
    mailbox_buffer[1] = MAIL_PROC_REQ;
...
const mailbox_vc_ops_t bcm2837_mailbox_vc_ops = {
    .read   = bcm2837_mailbox_read,
    .write  = bcm2837_mailbox_write,
    .get_firmware_version = bcm2837_get_firmware_version,
    .get_board_model = bcm2837_get_board_model,
    .get_board_revision = bcm2837_get_board_revision,
    .get_mac_address = bcm2837_get_mac_address,
    .get_board_serial = bcm2837_get_board_serial,
    .get_arm_memory = bcm2837_get_arm_memory,
    .get_soc_temperature = bcm2837_get_soc_temperature,
    .get_clock_rates = bcm2837_get_clock_rates,
    .get_clock_rates_measured = bcm2837_get_clock_rates_measured,
    .get_max_clock_rates = bcm2837_get_max_clock_rates,
    .get_min_clock_rates = bcm2837_get_min_clock_rates
};
