...
void bcm2711_interrupts_init_core0(void) {
    ...
    //    INT_GICC_2711->CTLR &= ~((1 << 5) | (1 << 6)); // Bypass enable for IRQ's and FIQ's
    INT_GICC_2711->PMR = 0xF0;              // Priority Mask: laat alle interrupts met prioriteit 0-255 door
    ...
    bcm2711_gic400_fiq_enable(GIC_IRQ_MINI_UART, CORE0); // Enable mini UART interrupt voor Core 0
    INT_GICD_2711->CTLR = 1;                // Schakel de Distributor in
    interrupts->irq_enable();               // Enable IRQs
    interrupts->fiq_enable();               // Enable FIQs
}
...
void bcm2711_interrupts_init_core1(void) {
    ...
    bcm2711_gic400_irq_enable(GIC_IRQ_MBOX0, CORE1); // Enable mailbox0 interrupt voor Core 1
    INT_GICC_2711->CTLR = 1;                // Schakel de CPU Interface in
